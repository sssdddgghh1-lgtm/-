...
# لوحة التحكم الرئيسية (تُستخدم للرسائل والأزرار)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ch":
        ...
        conn.commit()
        await query.edit_message_text("✅ تم تغيير حالة التشغيل", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="back")]]))
    elif data == "back":
        await menu(update, context, query=query)
    elif data == "post":
...
async def auto_post(app):
    while True:
        # هنا سنضيف منطق النشر التلقائي لاحقاً
...
async def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            POST: [MessageHandler(filters.ALL, handle_text)],
            INTERVAL: [MessageHandler(filters.TEXT, handle_text)],
            DELETE: [MessageHandler(filters.TEXT, handle_text)],
        },
